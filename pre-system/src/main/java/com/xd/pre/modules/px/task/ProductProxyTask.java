...
public class ProductProxyTask {
    ...
    public void productAll() {
        ...
        try {
            proxyProductService.productIpAndPort3();
        } catch (Exception e) {
...
//        }
    ...
    public Boolean notifySuccess(JdMchOrder jdMchOrder) {
        try {
            ...
            if (StrUtil.isNotBlank(no_time) && Integer.valueOf(no_time) >= 5 && jdMchOrderDb.getNotifySucc() == PreConstant.ONE) {
                log.info("订单号:{},已经通知3次成功没有必要再通知", jdMchOrder.getTradeNo());
                return true;
            }
